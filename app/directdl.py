...
def validate_schema(*parameters_to_check):
    def decorator(f):
        ...
        def wrapper(*args, **kw):
            ...
            if "type" in j.keys():
                if j['type'] not in ["album", "track", "album_track", "playlist", "artist", "artist_track"]:
                    return jsonify({"error": "type must be album, track, album_track, playlist, artist, or artist_track"}), 400
